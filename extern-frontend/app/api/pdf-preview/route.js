...
export async function GET(request) {
  try {
    ...
    const fullUrl = `${backendUrl}/invoices/${id}/pdf-preview?craftsman_id=${craftsman_id}`;
    
    console.log(`Forwarding PDF preview request to: ${fullUrl}`);
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: 'Failed to preview PDF' }, { status: response.status });
    }
